...
class MLP(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        print(80*'-')
        print(" print X values ", x.shape)
        print(x)
        print(self.c_fc.weight)
        print(80*'-')
    
        print("test")
        test_ = x @ self.c_fc.weight.T
        print( "tests results ", test_)
        print( " shape of test ", x.shape, self.c_fc.weight.T.shape )
        """
        
        print( " shape before c_fc(x) ", x.shape )
        print( " test [1] ", x[0][0][:10] )
        print( " show first 10 items for self.c_fc.weight.T ", self.c_fc.weight.T[:10])
        test_ = x @ self.c_fc.weight.T
        print( " do test and 10 to compare " , test_[:10])
        ...
        # need to keep x value as raw before x = self.c_fc(x)
        ###########
        x = self.c_fc(x)
        
        print(x.shape)
        #print( " weights c_fc ", x.weight.shape) # Error: 'Tensor' object has no attribute 'weight'
        print( " weights c_fc ", self.c_fc.weight.shape)
        print( " test [2] x = self.c_fc(x) ", x[0][0][:10] )
        print( " bias  self.c_fc ", self.c_fc.bias)
        ...
        #print( " Weights after x = self.c_fc(x) ==> ", x.weight)
        #print(" result ", x.shape , x)
        x = self.gelu(x)
        x = self.c_proj(x)
        #print(x.shape)
        x = self.dropout(x)
